...
public class EasyRedisTemplate {
	...
	public final boolean tryLock(String key, String value, Long expire) {
		return getRedisAdapter().tryLock(key, value, expire);
	}

	@Describe(value = "分布式锁-解锁，返回值:true-解除锁成功/false-解除锁失败 ")
	public final boolean unLock(String key, String value) {
		return getRedisAdapter().unLock(key, value);
	}
